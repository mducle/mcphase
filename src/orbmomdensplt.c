...
fprintf(fout,"#orbital moment density is expanded in tesseral harmonics Zlm\n\
#   ML(r).(%g,%g,%g)= sum_lm aL(l,m) F(r) Zlm(Omega)\n#\n\
#   with F(r)=1/r int_r^inf R^2(x) dx\n\
#   E. Balcar J. Phys. C. 8 (1975) 1581\n#\n ",xx,yy,zz);
   for(i=1;i<=49;++i){
...
if(gp.show_pointcharges>0) nofpc=read_pointcharge_parameters(gp,cs.cffilenames,argv[6+doijk],cs.x,cs.y,cs.z,jjjps,cs.abc);
  ...
  gp.spins_scale_moment=0;
  gp.spins_show_static_moment_direction=0;
  if(doijk==3) sprintf(gp.title,"projection of orbmomdensity ML(r).(%g,%g,%g)",xx,yy,zz);
...
fprintf(stderr,"# * saved density mesh in results/orbmomdensplt.grid\n");
fprintf(stderr,"# ************************************************************************\n");
