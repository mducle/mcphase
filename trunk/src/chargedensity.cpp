...
Vector & chargedensity::rtf(int p) // pointer to rft (i=1 ... nofpts)
{
  if (p>(*this).nofpts)
  {
    fprintf(stderr,"Error class chargedensity: point index %i exceed maximum nofpts %i \n",p,nofpts);
    exit (EXIT_FAILURE);
  }
  return (*rtetafi[p]);
}
...
chargedensity::chargedensity(double dt,double df)     //constructor
{
  ...
  for (i=1;i<=nofpts;++i)
  {
    rtetafi[i]=new Vector(1,3);(*rtetafi[i])=0;
  }     
  dtheta=dt;
  dfi=df;
}
...
chargedensity::chargedensity(const chargedensity & p) //kopier-konstruktor
{
  ...
  for (i=1;i<=nofpts;++i)
  {
    rtetafi[i]=new Vector(1,3);
    (*rtetafi[i]) = (*p.rtetafi[i]);
  }     
}
