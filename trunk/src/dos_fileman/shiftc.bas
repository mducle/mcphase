...
version$ = "<SHIFTC v 02.03.08 "
PRINT version$; COMMAND$;
IF LTRIM$(COMMAND$) = "" GOTO 333
DATA "*************************************************************************"
DATA "  program shiftc - use it like 'SHIFTC *.* 2 3.47'                       "
DATA "    (means add 3.47 to column 2) - if you want to shift more files       "
DATA "  with increasing the shift in each file add the option /+               "
...
PRINT #2, version$; " "; COMMAND$; DATE$; " "; TIME$;
PRINT #2, " SHIFTC>}"
...
WHILE EOF(1) = 0
 ...
 'add constant to  column jj%
 xm#(jj%) = xm#(jj%) + constant
...
WEND '<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
 ...
 CLOSE 1, 2
SHELL "copy shiftc.shi " + filename$
SHELL "del shiftc.shi"
PRINT
PRINT " SHIFTC>"
IF INSTR(COMMAND$, "/+") > 0 THEN constant = constant + VAL(a$)
IF INSTR(COMMAND$, "*") <> 0 GOTO 999
END

333 FOR i = 1 TO 17: READ a$: PRINT a$: NEXT: END
