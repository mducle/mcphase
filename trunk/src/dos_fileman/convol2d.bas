...
PRINT "CONVOL2D CONVOL2D CONVOL2D CONVOL2D CONVOL2D CONVOL2D CONVOL2D"
IF LTRIM$(COMMAND$) = "" GOTO 333
DATA "*************************************************************************"
DATA "program CONVOL2D-use it like CONVOL2D *.* 234 stp=0.014 mode=gauss fwhm=0.2"
DATA "    (means take file *.* second/third column as x/yaxis and calculate            "
DATA "    CONVOL2Dation for z axis=4th column  with stepwidth 0.014) -    "
DATA " ---> the result is written in file *.cvt                         "
DATA "   use the mode= switch to specify the type of CONVOL2Dion function     "
DATA "   used (gauss) and then specify the function parameters of f(x,y)    "
DATA "   CONVOL2Dion is done according to the formula:             "
...
DATA "*************************************************************************"
DIM text$(600), x#(1000), y#(1000), z#(1000), in#(30)
...
ix% = ASC(LEFT$(a$, 1)) MOD 48: iy% = ASC(MID$(a$, 2, 1)) MOD 48: iz% = ASC(MID$(a$, 3, 1)) MOD 48
...
PRINT #2, DATE$; " "; TIME$; " column "; ix%; "/"; iy%; "  was taken as the x/y axis for ";
PRINT #2, "2dim CONVOLuting the z axis with stepwidth "; stp;
PRINT #2, iz%; " was taken as the zaxis - CONVOL2Dion was done with a  "; mode$; "-function"
PRINT #2, "the command was: "; COMMAND$;
PRINT #2, "- used program: CONVOL2D.bas}"

REM write output data columns IN BLOCKS OF 1000 iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii
x#(0) = xmin#: y#(0) = ymin#
22 h% = 0
23        WHILE x#(h%) < xmax# AND h% < 1000  'setup block of xdata
             h% = h% + 1: x#(h%) = x#(h% - 1) + stp
             y#(h%) = y#(h% - 1): z#(h%) = 0
           WEND
           IF h% < 1000 AND x#(h%) >= xmax# AND y#(h%) < ymax# THEN
             h% = h% + 1: y#(h%) = y#(h% - 1) + stp
             x#(h%) = xmin#: z#(h%) = 0: GOTO 23
           END IF
...
IF LEFT$(mode$, 5) = "gauss" THEN    'GAUSS CONVOL2DION
   dd = ((x#(i%) - in#(ix%)) * (x#(i%) - in#(ix%)) + (y#(i%) - in#(iy%)) * (y#(i%) - in#(iy%))) / (fwhm * fwhm / 4 / LOG(.5))
 IF SQR(-dd) < 1E+19 THEN
   z#(i%) = z#(i%) + in#(iz%) * EXP(dd) / fwhm
 END IF
END IF
...
SHELL "del CONVOL2D.cvt"

PRINT
PRINT "END CONVOL2D in file "; filename$; " column "; ix%; "  was taken as the x axis for linear ";
PRINT "CONVOL2Dating the other axis with stepwidth "; stp;
IF iy% <> 0 THEN PRINT ", the "; iy%; " and "; coll%; "col were taken as the yaxis and curvature for CONVOL2Dating a cubic spline"
PRINT
...
'this sub calculates the newton CONVOL2Dation yy# at
...
'sum up newton CONVOL2Dation formula
