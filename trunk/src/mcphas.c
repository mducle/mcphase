...
int main (int argc, char **argv)
{
  ...
  ini.exit_mcphas=0;
  ini.print(); // if exit was 1 - save parameters and set exit=0
  ...
  {
    for (im=1; im<=inputpars.nofcomponents; ++im)
    {
      h1 = 0;
      h1(im) = 10 * MU_B * (*inputpars.jjj[l]).gJ;
      mmax(inputpars.nofcomponents * (l-1) + im) = (*inputpars.jjj[l]).mcalc(T,h1,z,u)(im);
    }
  }

  for (im=1; im<=inputpars.nofcomponents && im<=3; ++im)
  {
    mmax1(im) = mmax(im);
  }
  ...
  testspincf testspins (MAXNOFSPINCF,"./mcphas.tst","./results/mcphas.phs", \
                        inputpars.nofatoms,inputpars.nofcomponents);
  qvectors testqs (ini,inputpars.rez,mmax,"./results/mcphas.qvc", \
                        inputpars.nofatoms,inputpars.nofcomponents,verbose);
  ...
  if (argc>1 && strncmp(argv[argc-1],"-",1) != 0)
  {
    ini.xv = 0;
    ini.yv = 0;
    fin = fopen_errchk (argv[argc-1],"rb"); //input from file
  }
...
// loop different H /T points in phase diagram
  ...
  { //begin initialize display file
    ...
    { //correct some of input errors in parameters ini (if there are any)
      ...
      if (ini.xmin>ini.xmax)
      {
        dumm = ini.xmin;
        ini.xmin = ini.xmax;
        ini.xmax = dumm;
      }
      if (ini.ymin>ini.ymax)
      {
        dumm = ini.ymin;
        ini.ymin = ini.ymax;
        ini.ymax = dumm;
      ...
      {  // if yes -> input them
        while (feof(fin)==0 && 0==inputline(fin,nn));  
...
//calculate physical properties at HT- point
      ...
      if(fin != NULL)
      {
        y=ini.ymin - ini.ystep;
      } // this is to switch off xy loop if xy points are read from file
