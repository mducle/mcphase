...
//k = 11 / 10 * 11 / 9 * 11 / 8 * 11 / 7 * 11 / 6 * 11 / 5 * 11 / 4 * 11 / 3 * 11 / 2 * 11 / 1 * 11
  ...
  ro = ro + a(4, 4)  * st2 * st2 * \
       (cfi * cfi * cfi * cfi - 6 * cfi * cfi * sfi * sfi + sfi * sfi * sfi * sfi);

  ro = ro + a(6, -6) * st2 * st2 * st2 * (6 * cfi * cfi * cfi * cfi * cfi * sfi - \
       20 * cfi * cfi * cfi * sfi * sfi * sfi + 6 * cfi * sfi * sfi * sfi * sfi * sfi);
  ro = ro + a(6, -5) * ct * st * st2 * st2 * (5 * cfi * cfi * cfi * cfi * sfi - \
       10 * cfi * cfi * sfi * sfi * sfi + sfi * sfi * sfi * sfi * sfi);
  ...
  ro = ro + a(6, 4)  * (11 * ct2 - 1) * st2 * st2 * (cfi * cfi * cfi * cfi - \
       6 * cfi * cfi * sfi * sfi + sfi * sfi * sfi * sfi);
  ro = ro + a(6, 5)  * ct * st * st2 * st2 * (cfi * cfi * cfi * cfi * cfi - \
       10 * cfi * cfi * cfi * sfi * sfi + 5 * cfi * sfi * sfi * sfi * sfi);
  ro = ro + a(6, 6) * st2 * st2 * st2 * (cfi * cfi * cfi * cfi * cfi * cfi - \
       15 * cfi * cfi * cfi * cfi * sfi * sfi + \
       15 * cfi * cfi * sfi * sfi * sfi * sfi - sfi * sfi * sfi * sfi * sfi * sfi);
  ro = ro * rr;
...
//   if (instr[strspn(instr," \t")]=='#'){fprintf(fout,instr);}
    if(abc[1]==0)
    {
      extract(instr,"a",abc[1]);
      extract(instr,"b",abc[2]); 
      extract(instr,"c",abc[3]); 
    ...
    if (nofatoms > 0 && extract(instr,"x",x[n+1])+extract(instr,"y",y[n+1])+extract(instr,"z",z[n+1]) == 0)
    {
      ++n;
      if(n > nofatoms || nofatoms > MAXNOFATOMS)
      {
...
// load mfconfigurations and check which one is nearest -------------------------------   
                     ...
                     && (n=inputline(fin_coq,numbers)) >= 8;) // error in line reading (8 old format, 9 new format)
  {
     ...
     if (dd < delta)
     {
       delta = dd;
      sprintf(outstr, \
              "T=%g Ha=%g Hb=%g Hc=%g n=%g spins nofatoms=%i in primitive basis nofcomponents=%i", \
              numbers[3],numbers[5],numbers[6],numbers[7],numbers[8],(int)numbers[9],(int)numbers[10]);
      savmf=spins;
      T = numbers[3];
      ha = numbers[5];
      hb = numbers[6];
...
// cnst is the Zlm constants - put them into the matrix
  ...
  for(i=1; i<=savmf.na()*savmf.nb()*savmf.nc()*savmf.nofatoms; ++i)
    cd[i]=new chargedensity(dtheta,dfi);
...
// 1.b: mcalc has to be used to calculate all the <Olm>.
  ...
  for (i=1; i<=savmf.na(); ++i)
  {
    for(j=1; j<=savmf.nb(); ++j)
    {
      for(k=1; k<=savmf.nc(); ++k)
      {
        hh = savmf.m(i,j,k);
        ...
        {
          h=0;
          for(nt=1; nt<=savmf.nofcomponents; ++nt)
          {
            h(nt) = hh(nt+savmf.nofcomponents*(ii-1));
          }
...
//  tetan(2) = alpha: tetan(4) = beta: tetan(6) = gamma
          ...
          {
            for(m=-l; m<=l; ++m)
              a(l,m) *= tetan(l)*cnst(l,m)*cnst(l,m);
          }
...
//REM und zeichnet diese auf
          ...
          for(tt=0; tt<=3.1415/dtheta; ++tt)
          {
            for(ff=0;ff<=2*3.1415/dfi;++ff)
            {
              rp = 0;
              theta = (double)tt * dtheta;
              fi = (double)ff * dfi;
              nt = 0;
...
// extendedspincf.printall(fout,abc,r,x,y,z);
  ...
  fprintf(fout,"<thickness>0.0</thickness><color type=\"rgb\">255 0 0</color> \
