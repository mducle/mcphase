...
const char *szRCSID={"$Id: calib.cpp,v 1.1 2001/10/08 15:00:22 xausw Exp xausw $"};
...
{        int iO;
 ...
 while (iO!=EOF);
  ...
  if(!szArgV[optind]) // no file1 name; is it a pipe ?  
    {CreateTmpFile(szTFile, "Calib", TMPFILE_EXIT);
     if(atexit(RmTmpFile))
 ...
 enum EndLine  LT;
  ...
  if( (FileType) iType == FtNONE)
    {fprintf(stderr,"Calib: ERROR-exit\n");
     exit(EXIT_FAILURE);
    }
       ...
       switch ((FileGroup)(int)(iType/10)*10)
        {case TheGROUP:
              ...
              break;
       default:fprintf(stderr,"ERROR Calib: file %s has unsupported type %d",
                           szFile,iType);
          exit(EXIT_FAILURE);
      }//switch
  ...
  if(DF->GetFType()==FtERROR || DF->GetFType()==FtILLG)
    {fprintf(stderr,"ERROR Calib: Error reading file %s",szFile);
     exit(EXIT_FAILURE);
    }	    

  if(iSet>nSets)   
    {fprintf(stderr,"ERROR Calib: Illegal # of set %d (1...%d)\n",iSet,nSets);
     exit(EXIT_FAILURE);
    }		
  if( DF->ReadData(iSet)>0 || DF->GetCRData()->GetSteps()==0 )
    {fprintf(stderr,"ERROR Calib: Error reading data of %s\n",szFile);
     exit(EXIT_FAILURE);
    }
    
  if(iPrint)
   {String S;
    fprintf(stderr,"Calibrating The-File:%s Parameter file: %s ",
                    szFile,szPFile);
    DF->SPrintInfo(S,PR_ALL);
    fprintf(stderr,"%s",(const char *)S);    
   }

  if(Calibrate(DF,*FP)<=0)exit(EXIT_FAILURE);

  delete FP;
    
 if(LineT!=NOEnd)DF->SetLineType(LineT);
 if(!DF->SaveData(stdout))
   {fprintf(stderr,"ERROR Calib: Can not write to stdout\n");
    exit(EXIT_FAILURE);
   } 
...
{fprintf(stderr,"temp file:%s\n",szTFile);
 if(szTFile[0])
   {int i=remove(szTFile);
    ...
    if(i)perror("ERROR Calib: Removing tmp-file");
   }
