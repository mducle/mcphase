...
.
 ...
 foreach (@ARGV)
 {$file=$_; system ("copy ".$file." ".$file.".par"); open (Fin, $file);
   while($line=<Fin>)
        ...
        {$dpar=($parmax[$i]-$parmin[$i])/2**($searchlevel+2);
         if (abs($dpar)>abs($parstp[$i]))
         {
          ...
          if ($sta<$stamin){$stamin=$sta; foreach (@ARGV)
                                       {$file=$_; system ("copy ".$file." ".$file.".min.".$searchlevel);
                                        system ("copy ".$file.".par ".$file.".parmin.$searchlevel");}
			}
...
#        last if ($sta==0);
       if ($sta<$stamin){$stamin=$sta; foreach (@ARGV)
          ...
          foreach (@ARGV){$file=$_; system ("copy ".$file.".par ".$file.".$searchlevel.$minnumber");}
	  ++$minnumber;
       }
        }
}
...
close Fout;
 foreach (@ARGV) {$file=$_; system ("copy ".$file.".parmin.$searchlevel ".$file);}
...
#				  }
				   ...
				   foreach (@parnam)
				    ...
				    close Foutcc;
				    system "perl ccccccc.ccc > cccccc1.ccc";
				    open (Fincc,"cccccc1.ccc");
				    $data=<Fincc>; close Fincc;
				    system "del ccccccc.ccc cccccc1.ccc";
