...
//       est = ComplexMatrix(0,Hsz,0,Hsz); I comment this out - you should not reinitialize est !!!
      ...
      else  // Calculates using the Spectre method...
      {
//std::cout << "Using the spectre method\n";
         complexdouble *Vcso, *Hrot, *zmt, zalpha, zbeta; zalpha.r=1; zalpha.i=0; zbeta.r=0; zbeta.i=0; double *Ecso;
         // [Vcso,Ecso] = eig(ic_freehmltn(pars));
         sMat<double> Hcso = ic_Hcso(pars); Hcso/=MEV2CM; int Hcso_sz = Hcso.nc(); Ecso = new double[Hcso_sz]; Vcso = new complexdouble[Hcso_sz*Hcso_sz];
         sMat<double> zeros(Hcso_sz,Hcso_sz); int j = ic_diag(Hcso,zeros,Vcso,Ecso); if(j!=0) { std::cerr << "error: specre\n"; return; }
         for(i=0; i<Hcso_sz; i++) for(j=0; j<Hcso_sz; j++) if(fabs(Vcso[i+j*Hcso_sz].r)<DBL_EPSILON*50) Vcso[i+j*Hcso_sz].r=0.;
//for(i=0; i<Hcso_sz; i++) { for(j=0; j<Hcso_sz; j++) std::cout << Vcso[i+j*Hcso_sz].r << "+i" << Vcso[i+j*Hcso_sz].i << "\t\t"; std::cout << "\n"; }	 
         // Hrot = Vcso(:,1:pars.spectrelevels)'*Hcso*Vcso(:,1:pars.spectrelevels);
         char notranspose = 'N', transpose = 'C'; int cb = pars.spectrelevels; complexdouble *zHcso = zmat2f(Hcso,zeros);
         zmt = new complexdouble[Hsz*cb]; Hrot = new complexdouble[cb*cb]; 
         F77NAME(zgemm)(&notranspose,&notranspose,&Hcso_sz,&cb,&Hcso_sz,&zalpha,zHcso,&Hcso_sz,Vcso,&Hcso_sz,&zbeta,zmt,&Hcso_sz);
         F77NAME(zgemm)(&transpose,&notranspose,&cb,&cb,&Hcso_sz,&zalpha,Vcso,&Hcso_sz,zmt,&Hcso_sz,&zbeta,Hrot,&cb); free(zHcso); delete[]zmt;
//for(i=0; i<cb; i++) { for(j=0; j<cb; j++) std::cout << Hrot[i+j*cb].r << "+i" << Hrot[i+j*cb].i << "\t\t"; std::cout << "\n"; }
         // for i=1:lvl; J(i) = st_SO{find(abs(V(:,i))==max(abs(V(:,i))))}{5}; end
         std::vector<int> J2r(pars.spectrelevels,0), Jz2r, id; int imax,icv=0,mj; fconf conf(pars.n,0,pars.l);
         std::vector< std::vector<int> > cvSO2CF; std::vector<int> cvEl; int incx = 1;
         for(i=0; i<pars.spectrelevels; i++)
         {
            imax = F77NAME(izamax)(&Hcso_sz,&Vcso[i*Hcso_sz],&incx); J2r[i] = conf.states[imax-1].J2;   // IZAMAX returns fortran-style index (starts at 1)
            cvEl.push_back(icv); icv += J2r[i]+1; cvEl.push_back(icv-1); cvSO2CF.push_back(cvEl); cvEl.clear();
            for(mj=-J2r[i]; mj<=J2r[i]; mj+=2) { Jz2r.push_back(mj); id.push_back(i); }
         }
//std::cout << "J = " << dispvect(J2r) << "/2\n"; //std::cout << "id = " << dispvect(id) << "\n";
//std::cout << "cvSO2CF.size()==" << cvSO2CF.size() << "\t" << "icv==" << icv << "\n";
         complexdouble *Hrmj = new complexdouble[icv*icv]; convH2H(Hrot,Hrmj,cb,icv,cvSO2CF); delete[]Hrot;
         // Calculates the CF rotated matrix, and expands to |LSJ,mJ>
         int k,iq,qp,qm; sMat<double> UJ; complexdouble *fUJ, *UJr; double em,ep,icfact[]={0,0,-sqrt(15./7.)/2.,0,sqrt(11./14.),0,-sqrt(429./7.)/10.};
         for(k=2; k<=6; k+=2)
         {
            icfact[k] *= MEV2CM;
            UJ = racah_uJ(pars.n,k,pars.l); fUJ = zmat2f(UJ,zeros); UJr = new complexdouble[cb*cb]; zmt = new complexdouble[Hsz*cb];
            F77NAME(zgemm)(&notranspose,&notranspose,&Hcso_sz,&cb,&Hcso_sz,&zalpha,fUJ,&Hcso_sz,Vcso,&Hcso_sz,&zbeta,zmt,&Hcso_sz);
            F77NAME(zgemm)(&transpose,&notranspose,&cb,&cb,&Hcso_sz,&zalpha,Vcso,&Hcso_sz,zmt,&Hcso_sz,&zbeta,UJr,&cb); free(fUJ); delete[]zmt;
//std::cout<<"["; for(i=0; i<cb; i++) { for(j=0; j<cb; j++) std::cout << UJr[i+j*cb].r << "+i" << UJr[i+j*cb].i << "\t\t"; std::cout << "\n"; } std::cout<<"[";
            if(pars.B(k,0)!=0)
            {
               for(i=0; i<icv; i++) for(j=0; j<icv; j++)
               {
                  Hrmj[i+j*icv].r += pow(-1.,(J2r[id[i]]-Jz2r[i])/2.) * threej(J2r[id[i]],2*k,J2r[id[j]],-Jz2r[i],0,Jz2r[j]) 
                                         * UJr[id[i]+id[j]*cb].r * pars.B(k,0)/icfact[k];
               }
            }
//for(i=0; i<icv; i++) for(j=0; j<icv; j++) std::cout << "x("<< i+1 << "," << j+1 << ")=" << Hrmj[i+j*icv].r << "+sqrt(-1)*" << Hrmj[i+j*icv].i << "\n";
            for(iq=0; iq<k; iq++) if(pars.B(k,iq-k)!=0 || pars.B(k,-(iq-k))!=0)
            {
               qm = iq-k; qp = -qm;
               for(i=0; i<icv; i++) for(j=0; j<icv; j++)
               {
...
//double tr=0.,ti=0.; for(i=0; i<icv; i++) for(j=i; j<icv; j++) { tr+=fabs(Hrmj[i+j*icv].r-Hrmj[j+i*icv].r); ti+=fabs(Hrmj[i+j*icv].i+Hrmj[j+i*icv].i); }
//std::cout << "sum(sum(abs(Hrmj-Hrmj'))) = " << tr << "+i" << ti << "\n";
...
//tr=0.;ti=0.; for(i=1; i<=icv; i++) for(j=i; j<=icv; j++) { tr+=fabs(real(est[i][j])-real(est[j][i])); ti+=fabs(imag(est[i][j])+imag(est[j][i])); }
//std::cout << "sum(sum(abs(est-est'))) = " << tr << "+i" << ti << "\n";
//iceig VE; VE.calc(icv,Hrmj); for(i=0; i<icv; i++) std::cout << VE.E(i)-VE.E(0) << " "; std::cout << "\n";
       //for(i=1; i<=icv; i++) for(j=1; j<=icv; j++) est[i][j] = complex<double>(Hrmj[(i-1)+(j-1)*icv].r,Hrmj[(i-1)+(j-1)*icv].i);
...
//std::cout << "\n";
//for(i=0; i<Hcso_sz; i++) { for(j=0; j<Hcso_sz; j++) { if(fabs(Lrm[i+j*Hcso_sz].r)>1e-4) std::cout << Lrm[i+j*Hcso_sz].r; else std::cout << "0      ";
//if(fabs(Lrm[i+j*Hcso_sz].i)>1e-4) std::cout << "+i" << Lrm[i+j*Hcso_sz].i; std::cout << " \t"; } std::cout << "\n"; }
//std::cout << "\n";
//for(i=0; i<Hcso_sz; i++) { for(j=0; j<Hcso_sz; j++) { if(fabs(Srm[i+j*Hcso_sz].r)>1e-4) std::cout << Srm[i+j*Hcso_sz].r; else std::cout << "0      ";
//if(fabs(Srm[i+j*Hcso_sz].i)>1e-4) std::cout << "+i" << Srm[i+j*Hcso_sz].i; std::cout << " \t"; } std::cout << "\n"; }
//std::cout << "\n";
...
//std::cout<<"["; for(i=0; i<cb; i++) { for(j=0; j<cb; j++) std::cout << Lrj[i+j*cb].r << "+i" << Lrj[i+j*cb].i << "\t\t"; std::cout << "\n"; } std::cout<<"[";
//std::cout<<"["; for(i=0; i<cb; i++) { for(j=0; j<cb; j++) std::cout << Srj[i+j*cb].r << "+i" << Srj[i+j*cb].i << "\t\t"; std::cout << "\n"; } std::cout<<"[";
//for(i=0; i<cb; i++) { for(j=0; j<cb; j++) std::cout << Lrj[i+j*cb].r+2*Srj[i+j*cb].r << "+i" << Lrj[i+j*cb].i+2*Srj[i+j*cb].i << "\t\t"; std::cout << "\n"; }
         double sqrt2 = sqrt(2);
      // complexdouble *Lx,*Ly,*Lz,*Sx,*Sy,*Sz; Lz = new complexdouble[icv*icv]; Sz = new complexdouble[icv*icv];
      // Lx = new complexdouble[icv*icv]; Sx = new complexdouble[icv*icv]; Ly = new complexdouble[icv*icv]; Sy = new complexdouble[icv*icv];
         ...
         for(i=0; i<icv; i++) for(j=0; j<icv; j++)
         {
          ...
          //Lx[i+j*icv].r = (em-ep)*Lrj[id[i]+id[j]*cb]; Ly[i+j*icv].i = (-em-ep)*Lrj[id[i]+id[j]*cb];
          //Sx[i+j*icv].r = (em-ep)*Srj[id[i]+id[j]*cb]; Sy[i+j*icv].i = (-em-ep)*Srj[id[i]+id[j]*cb];
            FLx << (em-ep)*Lrj[id[i]+id[j]*cb].r << "\n"; FLy << (em+ep)*Lrj[id[i]+id[j]*cb].r << "\n";
            FSx << (em-ep)*Srj[id[i]+id[j]*cb].r << "\n"; FSy << (em+ep)*Srj[id[i]+id[j]*cb].r << "\n";
//Hrmj[i+j*icv].r -= (Lrj[id[i]+id[j]*cb].r+2.*Srj[id[i]+id[j]*cb].r)*.0578838263*(em-ep)/sqrt(3);
//Hrmj[i+j*icv].i -= (Lrj[id[i]+id[j]*cb].r+2.*Srj[id[i]+id[j]*cb].r)*.0578838263*(-em-ep)/sqrt(3);
            ep = pow(-1.,(J2r[id[i]]-Jz2r[i])/2.) * threej(J2r[id[i]],2,J2r[id[j]],-Jz2r[i], 0,Jz2r[j]);
          //Lz[i+j*icv].r = ep*Lrj[id[i]+id[j]*cb]; Sz[i+j*icv].r = ep*Srj[id[i]+id[j]*cb];
            FLz << ep*Lrj[id[i]+id[j]*cb].r << "\n"; FSz << ep*Srj[id[i]+id[j]*cb].r << "\n";
//Hrmj[i+j*icv].r -= (Lrj[id[i]+id[j]*cb].r+2.*Srj[id[i]+id[j]*cb].r)*.0578838263*ep/sqrt(3);
         }
//VE.calc(icv,Hrmj); for(i=0; i<icv; i++) std::cout << VE.E(i)-VE.E(0) << " "; std::cout << "\n";delete[]Hrmj;
   ...
   else
   {
      ...
      for(iq=J.Lo(); iq<=Jhi; iq++)
      {
         ...
         FILEIN.close();
//double tr=0.,ti=0.; for(i=0; i<icv; i++) for(j=i; j<icv; j++) { tr+=fabs(LS[iq-1][i+j*icv].r-LS[iq-1][j+i*icv].r); ti+=fabs(LS[iq-1][i+j*icv].i+LS[iq-1][j+i*icv].i); }
//std::cout << "sum(sum(abs(LS[" << iq << "]-LS[" << iq << "]'))) = " << tr << "+i" << ti << "\n";
      }
//double tr=0.,ti=0.; for(i=1; i<=icv; i++) for(j=i; j<=icv; j++) { tr+=fabs(real(est[i][j])-real(est[j][i])); ti+=fabs(imag(est[i][j])+imag(est[j][i])); }
//std::cout << "sum(sum(abs(est-est'))) = " << tr << "+i" << ti << "\n";
      complex<double> a(1.,0.); int incx = 1;
      for(i=1; i<=icv; i++) F77NAME(zaxpy)(&icv,(complexdouble*)&a,(complexdouble*)&est[i][1],&incx,&Jm[(i-1)*icv],&incx);
//tr=0.; ti=0.; for(i=0; i<icv; i++) for(j=i; j<icv; j++) { tr+=fabs(Jm[i+j*icv].r-Jm[j+i*icv].r); ti+=fabs(Jm[i+j*icv].i+Jm[j+i*icv].i); }
//std::cout << "sum(sum(abs(Jm-Jm'))) = " << tr << "+i" << ti << "\n";
      // Diagonalises the Hamiltonian H = Hic + sum_a(gjmbH_a*Ja)
      iceig VE; VE.calc(icv,Jm); delete[]Jm; 
//for(i=0; i<icv; i++) std::cout << VE.E(i)-VE.E(0) << " "; std::cout << "\n";
