		    ...
		    int &errflag)
{
  ...
  double oldfac, dv, newfac, sumbac, thresh, a1s, sumfor, sumuni, 
         sum1, sum2, x, y, m2, m3, x1, x2, x3, y1, y2, y3, cnorm, 
         ratio, a1, c1, c2, c1old, sign1, sign2;
  ...
  if (l1 - abs(m1) + eps < zero 
      || fmod(l1 + abs(m1) + eps, one) >= eps + eps) {
    errflag = 1;    
    Matpack.Error(Mat::ArgumentDomain, "%s: %s", "ThreeJSymbolM",
...
L10:
  // mscale = 0 
  thrcof[1] = (odd(int(abs(l2-l3-m1)+eps)) ? -one : one) / sqrt(l1+l2+l3+one);
  return;
...
L230:
  ...
  sign2 = odd(int(abs(l2-l3-m1)+eps)) ? -one : one;
