/***********************************************************
 *
 * orbmomdensplt - program to display spin density of
...
printf("***********************************************************\n");
printf("*\n");
printf("* orbmomdensplt - program to display spin density of \n");
...
// check command line
  if (argc < 7)
    { printf ("\nprogram orbmomdensplt - calculate orbmomdensity of a single ion\n\n\
                use as: orbmomdensplt threshhold T Ha Hb Hc i j k mcphas.sipf \n\n\
		...
		\n");
      printf ("\n\
...to view orbmomdensityplots type: javaview results/orbmomdensplt.jvx\n\
                                  displaycontour 2 3 4 results/orbmomdensplti.grid\n\
                                  displaycontour 1 3 4 results/orbmomdenspltj.grid\n\
                                  displaycontour 1 2 4 results/orbmomdenspltk.grid\n\
                \n");
      exit (1);
    }
...
if(xx!=0||doijk==0)jjjps.orbmomdensity_mcalc (momentsx,1, T, h, jjjps.mcalc_parstorage);
if(yy!=0||doijk==0)jjjps.orbmomdensity_mcalc (momentsy,2, T, h, jjjps.mcalc_parstorage);
if(zz!=0||doijk==0)jjjps.orbmomdensity_mcalc (momentsz,3, T, h, jjjps.mcalc_parstorage);
...
#   E. Balcar J. Phys. C. 8 (1975) 1581\n#\n ",xx,yy,zz);
   for(i=1;i<=49;++i){
   if(doijk>0){ printf(" aL(%i,%i) =%12.6f\n",k[i],q[i],moments(i));}
...
if(gp.show_pointcharges>0) nofpc=read_pointcharge_parameters(gp.scale_pointcharges,cs.cffilenames,argv[6+doijk],cs.x,cs.y,cs.z,jjjps,cs.abc);
  ...
  sprintf(gp.title,"orbmomdensity ML(r).(%g,%g,%g)",xx,yy,zz);
...
fprintf(stderr,"# * javaview results/orbmomdensplt.jvx\n");
fprintf(stderr,"# * displaycontour 2 3 4 results/orbmomdensplti.grid\n");
fprintf(stderr,"# * displaycontour 1 3 4 results/orbmomdenspltj.grid\n");
fprintf(stderr,"# * displaycontour 1 2 4 results/orbmomdenspltk.grid\n");
fprintf(stderr,"# ************************************************************************\n");
