...
int read_pointcharge_parameters(double scale,char ** cffilenames,char * cffile,float * x, float *y, float*z,jjjpar & jjjps,Vector & abc)
...
{if(jjjps.module_type==0||jjjps.module_type==4){printf("pointcharge %g |e| at xyz=%g %g %g mind: xyz=abc\n",par[1],par[2],par[3],par[4]);}
 ...
 ++nofpc;if(nofpc>MAXNOFATOMS){fprintf(stderr,"Error chrgplt - too many pointcharges");exit(1);}
  ...
  sprintf(cffilenames[1+nofpc],"pointcharge radius=%g",scale*0.529177*copysign(1.0,par[1])*pow((double)fabs(par[1]),0.3333));
