 ...
 FILE * fin_coq;
  ...
  for (tryrandom=0;tryrandom<=ini.nofrndtries&&myj!=0;++tryrandom)
   {if (myj>0){sps=(*(*thrdat.testspins).configurations[myj]);// take test-spinconfiguration
	     ...
	     if (tryrandom==0)
	     {nettom=(*thrdat.testqs).nettom(-myj);momentq0=(*thrdat.testqs).momentq0(-myj);phi=(*thrdat.testqs).phi(-myj);
	     }
	     else
	     {for(i=1;i<=(*myinput->inputpars).nofatoms;++i)
	      {for(ii=1;ii<=(*myinput->inputpars).nofcomponents;++ii)
	        {iii=(*myinput->inputpars).nofcomponents*(i-1)+ii;h1=0;h1(ii)=10*MU_B;
                 (*(*myinput->inputpars).jjj[i]).mcalc(mmom,thrdat.T,h1,lnz,u,(*(*myinput->inputpars).jjj[i]).mcalc_parstorage);
    ...
    if (tryrandom>0){nr=(int)(rint(rnd(1.0)*(sps.n()*(*myinput->inputpars).nofatoms-1)))+1;
		       ...
		       ri=(*myinput->inputpars).nofcomponents*(int)rint(rnd(1.0)*((*myinput->inputpars).nofatoms-1));
	               for(ii=1;ii<=(*myinput->inputpars).nofcomponents;++ii)
		       {sps.mi(rr)(ri+ii)=rnd(1.0) ;}
		       } // randomize spin rr
                    }
     if (H*sps.nettomagmom((*myinput->inputpars).gJ)<0) //see if nettomoment positiv
        {sps.invert();momentq0=-momentq0;} //if not - invert spinconfiguration

      //!!!calculate free eneregy - this is the heart of this loop !!!!
      mf=new mfcf(sps.na(),sps.nb(),sps.nc(),(*myinput->inputpars).nofatoms,(*myinput->inputpars).nofcomponents);
      fe=fecalc(H ,thrdat.T,(*myinput->inputpars),sps,(*mf),u,(*thrdat.testspins),(*thrdat.testqs));
      delete mf;
      ...
      if (fe<thrdat.femin)
	       ...
	       sps1=sps;sps1.reduce();
                   mf=new mfcf(sps1.na(),sps1.nb(),sps1.nc(),(*myinput->inputpars).nofatoms,(*myinput->inputpars).nofcomponents);
               if ((fered=fecalc(H ,thrdat.T,(*myinput->inputpars),sps1,(*mf),u,(*thrdat.testspins),(*thrdat.testqs)))<=fe+0.00001)sps=sps1;
                   delete mf;
                 // display spinstructure
                if (verbose==1)
                {Vector abc(1,3);
		 abc(1)=(*myinput->inputpars).a;
		 abc(2)=(*myinput->inputpars).b;
		 abc(3)=(*myinput->inputpars).c;
		 float x[(*myinput->inputpars).nofatoms],y[(*myinput->inputpars).nofatoms],z[(*myinput->inputpars).nofatoms];
		 for (is=1;is<=(*myinput->inputpars).nofatoms;++is)
		   {x[is]=(*(*myinput->inputpars).jjj[is]).xyz[1];
 		    y[is]=(*(*myinput->inputpars).jjj[is]).xyz[2];
		    z[is]=(*(*myinput->inputpars).jjj[is]).xyz[3];}
                     sprintf(text,"fe=%g,fered=%g<femin=%g:T=%gK, |H|=%gT,Ha=%gT, Hb=%gT, Hc=%gT,  %i spins",fe,fered,thrdat.femin,thrdat.T,Norm(H),H(1),H(2),H(3),sps.n());
                    fin_coq = fopen_errchk ("./results/.spins3dab.eps", "w");
                     sps.eps3d(fin_coq,text,abc,(*myinput->inputpars).r,x,y,z,4,(*myinput->inputpars).gJ);
                    fclose (fin_coq);
                    fin_coq = fopen_errchk ("./results/.spins3dac.eps", "w");
                     sps.eps3d(fin_coq,text,abc,(*myinput->inputpars).r,x,y,z,5,(*myinput->inputpars).gJ);
                    fclose (fin_coq);
                    fin_coq = fopen_errchk ("./results/.spins3dbc.eps", "w");
                     sps.eps3d(fin_coq,text,abc,(*myinput->inputpars).r,x,y,z,6,(*myinput->inputpars).gJ);
                    fclose (fin_coq);
  ...
  // log fe if required
   if (ini.logfevsQ==1) {
                 ComplexVector a(1,3*(*myinput->inputpars).nofatoms),b(1,3*(*myinput->inputpars).nofatoms);
                 ComplexVector b1(1,(*myinput->inputpars).nofcomponents*(*myinput->inputpars).nofatoms);
                 float inmax=0;int qh,qk,ql,l;
                 ComplexVector * mq;  
                 mq = new ComplexVector [sps.in(sps.na(),sps.nb(),sps.nc())+2];for(l=0;l<=sps.in(sps.na(),sps.nb(),sps.nc())+1;++l){mq[l]=ComplexVector(1,(*myinput->inputpars).nofcomponents*(*myinput->inputpars).nofatoms);}
                 Vector sq2(1,3*(*myinput->inputpars).nofatoms),qs(1,3),qt(1,3);float in;qs(1)=1000;
		 ...
		 // biggest Fourier component of the magnetic moment arrangement 
		   ...
		   b1 = mq[sps.in(sps.na()-qh,sps.nb()-qk,sps.nc()-ql)];
                   for(l=1;l<=(*myinput->inputpars).nofatoms;++l)
		   {int m1,m1max=3; if ((*(*myinput->inputpars).jjj[l]).gJ==0){m1max=6;}
		    for (m1=1;m1<=m1max;++m1)
		     {if((*(*myinput->inputpars).jjj[l]).gJ==0)
		      {if(m1==2||m1==4||m1==6){b(3*(l-1)+(m1+1)/2)+=b1((*myinput->inputpars).nofcomponents*(l-1)+m1);}
		       else                   {b(3*(l-1)+(m1+1)/2)+=2.0*b1((*myinput->inputpars).nofcomponents*(l-1)+m1);}
		      }
		      else
		      {b(3*(l-1)+m1)=b1((*myinput->inputpars).nofcomponents*(l-1)+m1)*(*(*myinput->inputpars).jjj[l]).gJ;
		      }
		     }    
		    }
		   a = b.Conjugate();
		   ...
		   // inner product
                   sq2=Abs(b+a)/(double)sps.n()/(double)(*myinput->inputpars).nofatoms;
                   Vector q(1,3);
		   q(1)=1.0*qh/sps.na();
	           q(2)=1.0*qk/sps.nb();
                   q(3)=1.0*ql/sps.nc();
                   qt=(*myinput->inputpars).rez.Transpose()*q;
