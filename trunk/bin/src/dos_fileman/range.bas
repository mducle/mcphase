...
PRINT "RANGE RANGE RANGE RANGE RANGE RANGE RANGE RANGE RANGE"
IF LTRIM$(COMMAND$) = "" GOTO 333
DATA "*************************************************************************"
DATA "  program RANGE - use it like RANGE *.* 2  1.23 3.45                    "
DATA "    (means delete all values out of range [1.23,3.45] in column 2
DATA "     from file *.*) -                                                 "
DATA " format of file                                                          "
DATA ""
...
jj% = ASC(LEFT$(a$, 1)) MOD 48 'get column
a$ = LTRIM$(RIGHT$(a$, LEN(a$) - INSTR(a$, " "))): ll = VAL(LEFT$(a$, INSTR(a$, " ")))
a$ = LTRIM$(RIGHT$(a$, LEN(a$) - INSTR(a$, " "))): ul = VAL(a$)
IF ll > ul THEN ls = ll: ll = ul: ul = ls
...
PRINT #2, DATE$; " "; TIME$; "data points out of ["; ll; ":"; ul; "] in column"; jj%;
PRINT #2, "have been deleted using program RANGE.bas}"
...
WHILE EOF(1) = 0
 ...
 IF xm#(jj%) <= ul AND xm#(jj%) >= ll AND col% >= jj% THEN
...
WEND
 ...
 CLOSE 1, 2
SHELL "copy RANGE.del " + filename$
SHELL "del RANGE.del"

PRINT
PRINT "END RANGE in file "; filename$; "data points out of ["; ll; ","; ul; "] in "; jj%; "have been deleted"
