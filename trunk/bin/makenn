...
print "              kf should be the Fermi wavevector (usually\n";
print "              between 1-2 A^-1 depending on the electrondensity^0.333)\n";
...
if(/-kaneyoshi/)
  {$rkky=2;shift @ARGV; $scale=$ARGV[0];shift @ARGV;$D=$ARGV[0];shift @ARGV;$aa=$ARGV[0];shift @ARGV;
   ...
   print "J(R)= A [-(R/D)^2+(R/D)^4].exp[-alpha.(R/D)^2] for scale A=$scale meV kf=$D A^-1  alpha=$aa\n";}
$_=$ARGV[0];
if(/-d/)
