...
if($sta>0)
{
 ...
 if ($sta>$stasave)
  ...
  {$stattemp=$stattemp*0.995;
   ...
   $hx=0;
   for($hx=0;$hx<=int(($parmax[$i]-$parmin[$i])/$parstp[$i])+1;++$hx)
   {print Fout (($hx+0.5)*$parstp[$i]+$parmin[$i])."   ".($parhisto[($hx+$perlhistostart[$i])])."\n";
   } close Fout;
...
# move files
 foreach (@ARGV)
 {$file=$_; system ("del $file.fit");
            system ("rename ".$file." ".$file.".fit");
            system ("del $file");
            system ("rename ".$file.".par ".$file);}
exit 0;
# END OF MAIN PROGRAM
#****************************************************************************** 
...
sub catch_zap {
 my $signame = shift;
 foreach (@ARGV)
 {$file=$_; system ("rename ".$file." ".$file.".fit");
            system ("rename ".$file.".par ".$file);}
