...
- crystal field parameter notation for the low symmetry parameters
  (m<0) BlmS and LlmS in so1ion/cfield/ic1ion/icf1ion
  has been adapted to literature usage. In previous McPhase versions
...
\right ]
            ...
            && -\frac{1}{2}  \sum_{nn'} {\mathcal J_S}(nn') {\mbf S}^n{\mbf S}^{n'}
               -\frac{1}{2}  \sum_{nn'} {\mathcal J_L}(nn') {\mbf L}^n{\mbf L}^{n'} \nonumber\\
...
\bibliography{li110830}   % bibtex - ing the database
