...
Lu$_2$MnO$_4$ &  magnetic structure and dispersion of excitations & {\prg so1ion} & \cite{lewtas10-184420} \\
...
$T_{l0}=\sqrt{4\pi/(2l+1)}\sum_iY_{l0}(\Omega_{i_s})$, 
$T_{l,\pm|m|}=\sqrt{4\pi/(2l+1)}\sum_i \sqrt{\pm1}[Y_{l,-|m|}(\Omega_{i_s})\pm (-1)^m Y_{l,|m|}(\Omega_{i_s})]$ for the ion $s$,
for details on crystal field parameter conventions see~\ref{cfparconventions}.
...
\bibliography{li110516}   % bibtex - ing the database
