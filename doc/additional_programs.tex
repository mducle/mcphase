...
\begin{description}
\item [\prg acoscol\index{acoscol} col file:] calculates arccosine of a column arccos(col) 
\item [\prg acoscolerr\index{acoscoler} col colerr file:] calculates arccosine of a column arccos(col)  and an errorcolumn colerr 
\item [\prg add\index{add} colx1 coly1 file1 colx2 coly2 file2:] program to add functions y1(x1) and y2(x2)
       taken from data file1 and data file2.
\begin{description}
 \item [ input:]
\item[  file1, file2 ]    filennames 
\item[colx, coly ]   columns containing x and y=f(x) values
\item[  output:]
...
\item [\prg adderr\index{adderr} colx1 coly1 coly1err file1 colx2 coly2 coly2err file2:]
   program to adderr functions y1(x1) with y1err and y2(x2) with y2err
       taken from data file1 and data file2
...
\end{description}
  note:            colx2 has to be sorted in file2
\item [\prg addcol\index{addcol}  colx coly file:] adds column x and column y of file, result is %%@
stored in col y
\item [\prg addcolerr\index{addcolerr}  colx colxerr coly colyerr file:]
 adds column x and column y of file, result is 
stored in col y. Error is added using the error columns colxerr and colyerr
...
\item [\prg displayerr\index{displayerr} colx coly colerr filename:]  program displayerr - show and watch data file by viewing a xy graphic on screen.     
        xcol,ycol,intcol ... column to be taken as x-, y- and yerrorbar axis.
            if intcol=- then lines are shown (no symbols and no errorbars)
...
\item [\prg factcolerr\index{factcolerr} col colerr const file:] multiplies col and colerr
 with a constant in file
