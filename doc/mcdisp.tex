...
%\includegraphics[angle=-90, width=0.8\textwidth]{../examples/ndcu2b/resultss/compF3.ps}
%\end{center}
...
The eigenvector matrix $\mathcal T$ provides a unitary transformation ($\mathcal T \mathcal T^\dag\Lambda=\mathcal %%@
T^\dag\Lambda\mathcal T=1$
and $\Lambda \mathcal T \Omega \mathcal T^\dag\Lambda=A$) of equation (\ref{psi0rpa}):

\begin{equation}
\delta_{ss'}=\sum_{s''rr's'''s''''}\Lambda_{ss''} \mathcal T_{s''r} (\hbar\omega_r-\hbar\omega)\delta_{rr'}\mathcal %%@
T^\dag_{r's'''}\Lambda_{s'''s''''}\Psi^{s''''s'}({\mbf Q},\omega)
\end{equation}

This equation may be easily inverted to provide the following expression of $\Psi^{ss'}({\mbf Q},\omega)$:

\begin{equation}\label{psiresult}
\Psi^{ss'}({\mbf Q},\omega)
=
\sum_{rr'} \mathcal T_{sr} (\hbar\omega_r-\hbar\omega)^{-1}\delta_{rr'}\mathcal T^\dag_{r's}
\end{equation}
...
\mathcal U^s_{\alpha1} \mathcal T_{sr}(\mbf Q) \frac{\omega_r(\mbf Q)}{|\omega_r(\mbf Q)|}\delta(\hbar \omega_r(\mbf Q) %%@
...
H=0 and T=1.5~K [plot created by program {\prg java javaview \index{javaview}}]
}
\end{figure}
...
\sqrt{\gamma^s}^\star \mathcal U^s_{\alpha1} \mathcal T_{sr}(\mbf Q)  \mathcal T^{\dag}_{r1}(\mbf Q) \mathcal %%@
...
\sqrt{\gamma^s}^\star \mathcal U^s_{\alpha1} \mathcal T_{sr}(\mbf Q)=
...
\pi \sum_r 
\mathcal V^s_{\alpha1} \mathcal T_{sr}(\mbf Q)\frac{\omega_r(\mbf Q)}{|\omega_r(\mbf Q)|} \delta(\hbar \omega_r(\mbf Q) %%@
