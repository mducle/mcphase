...
# Configuration
# (i)either specify single ion type by IONTYPE reading internal values for ion parameters XI 
F0 F2 F4 F6: 
#IONTYPE=Co2+
...
# from here on follow some OPTIONAL PARAMETERS
...
# to allow output of more than 4 components of the eigenvector in ic1ion.out, you need to put a flag:
# eigenvectors=X

# --- some faster algorithms for matrix diagonalisation ...

# keyword "save_matrices" - if uncommented then matrices which are often used
# are not recalculated each time, but saved in directory results/mms
# save_matrices


# if the next line is uncommented then it will use 
...
#there is a matrix truncation routine in mcalc() - which is
#triggered when you put:
#truncate_matrix=x
#   ... this routine first calculates and diagonalises the singleion
# Hamiltonian without the Zeeman or mean-field terms, and then uses
# the eigenvectors to make a unitary transformation matrix to rotate
# the multipolar and L and S operators into this new basis. Then it
# truncates the matrix, leaving only the x-fraction lowest terms
# (e.g. for f2 if you specify x=0.2, it would keep 20% of the levels,
# there are 91 levels so it would use 18 levels). This seems to be
# much faster (for low values of x) even than the arnoldi/lanczos method,
# but then it is much more approximate...
# Note The most computationally extensive operations are carried out at the
#start - the diagonalisation of the single ion Hamiltonian, and
#matrix-matrix multiplication to rotate the operator matrices. Once
#done the rotated matrices are stored in memory and then reused when
# the <J> expectation values have to be evaluated...
# the routine uses the mcalc_parstorage matrix for storing the rotated
# matrices.
# one should experiment with the
#value of x and compare it with using the full matrix diagonalisation
#(or the arnoldi/lanczos) one before relying on the results.
#This approximation method is probably not necessary for d-electron
#matrices (as those are generally quite small), but for f-electrons it
#is quite useful!
...
# There is also an experimental feature to use a truncated Hamiltonian matrix in
# mcalc() to speed up the calculation in mcphasit... You specify in the
# sipf a variable, "truncate_matrix=XX" where XX is between 0 and 1, and
# is the fraction of levels to keep... E.g. truncate_matrix=0.2 would
# keep only the lowest 5th set of energy levels. For f5 this is about
# 400 levels out of 2000. Unfortunately it's only for Linux at the
# moment, because I have to make a rotation matrix to change the basis
# of the Hamiltonian, and I store this matrix as a memory mapped file
# and the implementation of memory mapping is different from Linux to
# Windows.

# another experimental (not yet working) feature is for the calculation
# of spin and orbital magnetisation density in the three spatial directions
...
to the crystal field (CF) interaction which \emph{cfield} treats. 
...
What is refered to as \emph{intermediate coupling} can mean in general that all the three main
...
calculations. In these cases there is usually still a multiplet structure but the eigenstates of the
system is a mixture of the $LS$-basis states, with only $J$ being a good quantum number which can be
used to distinguish the states. In the case of a large CF interaction, however, \emph{J-mixing} can
occur, which results in the lost of a distinct multiplet structure.
...
by Allison (http://cpc.cs.qub.ac.uk/cpc/cgi-bin/showversions.pl/?catid=acry
).
...
\[ \mathcal{H}_{\mathrm{coulomb}} = \sum_{i>j=1}^N \frac{e^2}{|\mathbf{r}_i-\mathbf{r}_j|} = e_0 E^0 + e_1 E^1
+ e_2 E^2 + e_3 E^3 \]
...
\item[The magnetic moment operators, $L_x L_y L_z S_x S_y S_z$:]
