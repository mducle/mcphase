...
full $l^n$ configuration with many manifolds, the operator equivalent factors only serve to convert the input
Stevens parameters into Wybourne normalisation, which is used internally by the program to calculate the
Hamiltonian.

If the user wishes to compare the output of {\prg ic1ion} with published energies/wavefunctions using Stevens
parameters for rare earths, they should set the Slater {\prg F2, F4, F6} and spin-orbit {\prg xi} integrals to
very high values - such as 10$^7$ meV, in order to obtain the $LS$-coupling limit. For comparison with Stevens
parameters within an $L$-manifold for $d$-electron ions, the user should set the Slater integrals to high
...
{\prg xi} is specified, then this degeneracy will be lifted.

In addition to the usual Stevens, $B_l^m$, and Wyborne $L_l^m$ parameters noted above, {\prg ic1ion} also
understands certain variants to these parameters using the same notation as {\prg cfield}/{\prg so1ion}:
...
\begin{center} \begin{tabular}{@{\extracolsep{\fill}}r|l|l} \hline
     ...
     Blm        & = sign($m$) $ \lambda_{lm} \langle J || \theta_l || J \rangle$ & Stevens \\
     ARlm       & =           $ \lambda_{lm} L_l^m $                             & Stevens \\
     Wlm        & = half($m$) $ \lambda_{lm} L_l^m /\langle r^l \rangle$         & Stevens \\
     Vlm        & = half($m$) $ \lambda_{lm} \langle J || \theta_l || J \rangle$ & Stevens \\
     Llm        & =           $              L_l^m $                             & Wybourne \\
     Dlm        & =           $              L_l^m $                             & Wybourne \\ \hline
...
\begin{table}[h] \renewcommand{\arraystretch}{1.3}
  \begin{center}
   ...
   %  \multicolumn{3}{|c|}{$k$~~~$q$~~~$\lambda_{kq}$} \\ \hline
   %  $k$  &  $q$  &  $\lambda_{kq}$ \\ \hline \hline
   %  $2$  &  $0$  &  $\frac{1}{2}$  \\
   %  $2$  &  $1$  &  $-\sqrt{6}$  \\
   %  $2$  &  $2$  &  $\frac{1}{2}\sqrt{6}$  \\
   %  $4$  &  $0$  &  $\frac{1}{8}$  \\
   ...
   %  $6$  &  $6$  &  $\frac{1}{16}\sqrt{231}$  \\ \hline
    \begin{tabular}{c|ccccccc}
     ...
     %\backslashbox{$k$}{$|q|$} & $0$ & $1$ & $2$ & $3$ & $4$ & $5$ & $6$ \\
      ...
      \hline
    \end{tabular}
    \caption{\emph{Ratios of the Stevens to the Wybourne parameters}. $\lambda_{lm} = A_{lm}\langle r^l \rangle / L_l^m$.} \label{tab:wytostev}
  \end{center}
\end{table}
