...
eigenvector $u1$, which is given by equation (\ref{ufirstrow}): ${\mbf u^s_{\alpha1}}=\sqrt{(n_--n_+)}<-|J^s_{\alpha}-<J^s_{\alpha}>_{\mbf H,T}|+>$. Note that in contrast to ${\mathcal U^s_{\alpha1}}$ the eigenvector
...
\item for the transition number tn the vector u1 is to be filled with ${\mbf u^s_{\alpha1}}=\sqrt{(n_--n_+)}<-|J^s_{\alpha}-<J^s_{\alpha}>_{\mbf H,T}|+>$.
\item if the a negative value for T is entered, the vector u1 is to be filled with
        ${\mbf u^s_{\alpha1}}=\sqrt{(n_--n_+)}<-|J^s_{\alpha}|+>$ and the 
...
output should be a corresponding vector
 ${\mbf v^s_{\alpha1}}(\mbf Q)=\sqrt{(n_--n_+)}<-|\mathcal Q^{\dag}_{\alpha}|+>$.
