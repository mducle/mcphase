...
{print "program fit: simple function fit to data

    use as fit coldata prog col parprog1 parprog2 ... []  in filename
...
{@prog=split;
 ...
 foreach(@prog)
  {$old=$prog[$j];
   ...
   $prog[$j]="par".$i.$program.$j."[".$old.",-1e20,+1e20,0,$step]";
  }
 $pars=join(' ',@prog);
 $i++;print Fout "call ".$program." $i ".$pars." range1.out\n";
}
...
$i++;system("addcol ".$prog[1]." $i range0.out");
   system ("display ".$prog[1]." $coldata $file.fit ".$prog[1]." ".($coldata+$#progs+2)." $file.fit");
 }
   ...
   copy ("calcsta.bat","results/calcsta.bat");
