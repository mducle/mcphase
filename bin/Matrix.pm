...
#
# Copyright (C) 2013, Martin Rotter
...
#
# Revision 0.6  2013/03/02 
# Martin Rotter added ** functionality
#
...
Calculates the power of a matrix fo positive integers.
...
=head2 as_string
returns string for printing a  matrix. Complex 
numbers are written as a + i b.

=head2 as_block

returns string for printing a complex matrix in separate blocks 
...
=head1 EXAMPLE
        ...
        # set an element (mind index starts at 0,0)!
        $a->[1][1]=4.5;
        print "A\n$a";
...
Martin Rotter E<lt>martin.rotter@cpfs.mpg.deE<gt>
=cut
...
sub as_string
    {   my $self = shift;             
        ...
        for (@$self)
        {
            ...
            for (@$_)
            {   my $s=$_;if (ref($s)){
                my ($re, $im) = @{$s->{cartesian}};
                push @z, sprintf "%s%.*f%s%.*fi", ($re < 0 ? '' : ' '), $Precision, $re,
                                                  ($im < 0 ? '' : '+'), $Precision, $im;
                                          }
                else
                {push @z,sprintf "%s%.*f", ($s < 0 ? '' : ' '), $Precision, $s;}
            }
            push @ret, join(' ', @z), "\n";
        }
      return join('', @ret);
...
sub as_blocks
    {   my $self = shift;             
        ...
        for (@$self)
        {
            ...
            {   my $s=$_;if (ref($s)){
                my ($re, $im) = @{$s->{cartesian}};
                push @z, sprintf "%s%.*f", ($re < 0 ? '' : ' '), $Precision, $re;
                                          }
                else
                {push @z,sprintf "%s%.*f", ($s < 0 ? '' : ' '), $Precision, $s;}
            }
       ...
       for (@$self)
        {
            ...
            {   my $s=$_;if (ref($s)){
                my ($re, $im) = @{$s->{cartesian}};
                push @z, sprintf "%s%.*f", ($im < 0 ? '' : '+'), $Precision, $im;
                                          }
                else
                {push @z,sprintf "%s%.*f", ($s < 0 ? '' : ' '), $Precision, 0.0;}
            }
            push @ret, join(' ', @z), "\n";
        }
      return join('', @ret);
