...
PRINT "SWAP SWAP SWAP SWAP SWAP SWAP SWAP SWAP SWAP SWAP SWAP SWAP SWAP"
IF LTRIM$(COMMAND$) = "" GOTO 333
DATA "*************************************************************************"
DATA "  program swap - use it like 'SWAP *.* 23'                               "
DATA "    (means swap col 2 and 3) -                                           "
...
PRINT #2, DATE$; " "; TIME$; " col "; ii%; "and col"; jj%; "have been swapped";
PRINT #2, " using program swap.bas}"
...
WEND'<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
 ...
 CLOSE 1, 2
SHELL "copy swap.swp " + filename$
SHELL "del swap.swp"
PRINT
PRINT "END SWAP in file "; filename$; " column "; jj%; "and "; ii%; " have been swapped"
