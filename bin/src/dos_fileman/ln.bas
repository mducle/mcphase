...
PRINT "LN LN LN LN LN LN LN LN LN LN LN LN LN"
IF LTRIM$(COMMAND$) = "" GOTO 333
DATA "*************************************************************************"
DATA "  program LN - use it like LN *.* 2"     
DATA "    (means calc LN of col 2:col2 = LN(col2) -                                "
...
PRINT #2, "LN calc using program LN.bas}"
...
WEND
 ...
 CLOSE 1, 2
SHELL "copy LN.LN " + filename$
SHELL "del LN.LN"
PRINT
SHELL "echo END LN in file " + filename$ + " column " + STR$(jj%) + " has been LNonantiated"
...
klzu% = INSTR(ala$, "}")
col2% = 0
WHILE klauf% < klzu% AND klauf% > 0   'take out closed bracketed LNressions
 col2% = col2% + 1
