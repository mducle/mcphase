...
PRINT "FIT1HET FIT1HET FIT1HET FIT1HET FIT1HET FIT1HET FIT1HET FIT1HET FIT1HET FIT1HET"
IF LTRIM$(COMMAND$) = "" GOTO 333
DATA "*************************************************************************"
DATA "  program FIT1HET - use it like FIT1HET *.* 23"
DATA " the HET function ist the resolution function of HET:"
DATA " f(x)  = H*theta(x + fwhm)[ theta(-x+fwhm-1/k)(1-abs(x)/fwhm)"
DATA "                           +theta(x-fwgm+1/k)/fwhm/k*exp(-k(x-fwhm/k)-1) ]"
DATA "    (23 means calc.1-lineHETfit of 3rd columns with respect to second column) - "
DATA " ---> the result is written in file *.*,    /G..... view graphics when fitting  "
DATA " the HET fit  is calculated by least squares method              "
DATA " the result is written to the last column of file *.*         "
DATA " /0=43  fix center of line at 43
...
OPEN "o", 2, "FIT1HET.lin"
PRINT #2, "{";
PRINT #2, "fitp:";
FOR i% = 1 TO par#(0): PRINT #2, USING "##.###^^^^"; par#(i%); : NEXT i%
PRINT #2, USING "sta=##.###^^^^"; sta#
FOR iii = 1 TO j: PRINT #2, text$(iii): NEXT
PRINT #2, DATE$; " "; TIME$; " column"; jj%; "FIT1HET calc. with respect to column"; ii%;
PRINT #2, "fit put into column"; col% + 1
PRINT #2, "calculation done by program FIT1HET.bas";
PRINT #2, "}"
...
OPEN "a", 1, "fit1HET.log"
IF LOF(1) < 2 THEN PRINT #1, "{par#(1) vs par#(2) vs ... vs sta vs av(1)(average of columns 1) vs av(2) vs ... vs filename (logfile of fit1HET.log)}"
...
SHELL "copy FIT1HET.lin " + filename$
SHELL "del FIT1HET.lin"
PRINT
PRINT "END FIT1HET in file "; filename$;
PRINT " column"; jj%; "FIT1HET calc. with respect to column"; ii%;
...
'one HETian
par#(0) = 4   '3 parameters:
              '1...center 2...fwhm 3...height 4...exponent k
              '(if /B switch is set: 5...constant background)
fwhm = par#(2)
a# = -4 * LOG(.5)
IF par#(2) = 0 THEN
 y# = 0
...
'DATA "                           +theta(x-fwgm+1/k)/fwhm/k*exp(-k(x-fwhm/k)-1) ]"
 IF dx# < -fwhm THEN
  y# = 0
 ELSE
  IF dx# < fwhm - 1 / par#(4) THEN
   y# = par#(3) * (1 - ABS(dx#) / fwhm)
  ELSE
   y# = par#(3) / fwhm / par#(4) * EXP(-par#(4) * (dx# - fwhm) - 1)
  END IF
 END IF
END IF

IF INSTR(COMMAND$, "/B") > 0 THEN par#(0) = 5: y# = y# + par#(5)
...
nn% = INSTR(COMMAND$, "/0=")
IF nn% > 0 THEN c0 = VAL(MID$(COMMAND$, nn% + 4)): par#(1) = c0: parmax(1) = c0: parmin(1) = 0
...
IF INSTR(COMMAND$, "/B") > 0 THEN
 par#(5) = bkg
 parstp(5) = .1 * ABS(sumy / n)
 parmax(5) = bkg + SQR(sumyy) / n
 parmin(5) = bkg - SQR(sumyy) / n
END IF
NEXT trytwice%
