...
void trs_header_out(FILE* fout,double & pinit,double & ninit,double & maxE,double & T,Vector & Hext)
{time_t curtime;
 struct tm *loctime;
...
int trs_write_next_line(FILE * fout,jjjpar & jjj,int & nt,int  i,int  j,int  k,int  l,int & tc,double & T,Vector & mf,
                     Vector & Hext,ComplexMatrix & est,float & d,double  minE,double  maxE)    
    ...
    while (minE>d||d>maxE) //only consider transition if it is in interval minE/maxE
     ...
     //fprintf(stdout,"transition number %i: ",jjj.transitionnumber);
    ...
    gamma=Norm2(u1);
     ...
     if(minE<d&&d<maxE)
    { fprintf(fout,"%i %i %i  %i     %i     %g  %g  %g\n",i,j,k,l,jjj.transitionnumber,myround(d),myround(gamma),myround(intensityp));
     ++tc;}
    if(d>=0&&minE<-d&&-d<maxE) // do not print negative energy transition if d<0 (d<0 means transiton to the same level)
    { fprintf(fout,"%i %i %i  %i     %i     %g  %g  %g\n",i,j,k,l,jjj.transitionnumber,myround(-d),myround(gamma),myround(intensitym));
    ++tc;}
 return 0;
} //write next line  
