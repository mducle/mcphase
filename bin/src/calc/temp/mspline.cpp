...
const char *szRCSID={"$Id: mspline.cpp,v 1.1 2001/10/08 15:00:22 xausw Exp xausw $"};
...
{int iO;
 ...
 while (iO!=EOF);
  ...
  if(!szArgV[optind]) // no file1 name; is it a pipe ?  
    {CreateTmpFile(szTFile, "Spline", TMPFILE_EXIT);
     if(atexit(RmTmpFile))
 ...
 enum EndLine  LT;
  ...
  if( (FileType) iType == FtNONE)
    {fprintf(stderr,"Spline: ERROR-exit\n");
     exit(EXIT_FAILURE);
    }
       ...
       switch ((FileGroup)(int)(iType/10)*10)
        ...
        case NoGROUP:
	     ...
	     nSets=DF->GetNSets();
             break;
       default:fprintf(stderr,"ERROR Spline: file %s has unsupported type %d",
                           szFile,iType);
          exit(EXIT_FAILURE);
      }//switch
  ...
  if(DF->GetFType()==FtERROR || DF->GetFType()==FtILLG)
    {fprintf(stderr,"ERROR Spline: Error reading file %s",szFile);
     exit(EXIT_FAILURE);
    }	    

  if(iSet>nSets)   
    {fprintf(stderr,"ERROR Spline: Illegal # of set %d (1...%d)\n",iSet,nSets);
     exit(EXIT_FAILURE);
    }		
  if( DF->ReadData(iSet)>0 || DF->GetCRData()->GetSteps()==0 )
    {fprintf(stderr,"ERROR Spline: Error reading data of %s\n",szFile);
     exit(EXIT_FAILURE);
    }
  ...
  if(iPrint)
   {String S;
    fprintf(stderr,"Save spline data of file:%s x-Column:%d y-Column:%d\n",szFile,ix,iy);
 ...
 if(!DF->SaveSplineTable(stdout,ix-1,iy-1))
   {fprintf(stderr,"ERROR Spline: Illegal column number\n");
    exit(EXIT_FAILURE);
   } 
...
{fprintf(stderr,"temp file:%s\n",szTFile);
 if(szTFile[0])
   {int i=remove(szTFile);
    ...
    if(!i)perror("ERROR Spline: Removing tmp-file");
   }
