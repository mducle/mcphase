...
/*
 * $Id: findmac.cpp,v 1.1 2001/10/08 15:00:22 xausw Exp xausw $
 */
...
void exit_nicely(PGconn *conn, int iexit);
  ...
  for (dip = (myIP & myNETMASK) + 1; dip < myBROADCAST; dip++)
    {
      ...
      memcpy (arp_pkt.dst_mac, "\377\377\377\377\377\377", 6); /* 00:06:66:03:13:37 :) */
	...
	{//printf ("*> Host %s, %s\n",
         ...
         //	  hwaddr (arp_pkt.sender_eth));
           S.Setf("%s",inetaddr (sip));
           IpA.InsertLine(S);
           S.Setf("%s",hwaddr (arp_pkt.sender_eth));
           Mac.InsertLine(S);
...
//printf("%d Ips and %d Macs\n",IpA.GetNLines(),Mac.GetNLines());
  if(IpA.GetNLines()!=Mac.GetNLines())
    {fprintf(stderr,"Number of Ips!=Macs\n");
     exit(EXIT_FAILURE);
    }
    ...
    {fprintf(stderr, "Connection to database '%s' failed.\n", dbName);
     ...
     exit_nicely(conn,EXIT_FAILURE);
    }

  
    int iTup;
    long unsigned iCnt;
    for(i=0;i<IpA.GetNLines();i++)
       {S.Setf("SELECT * FROM watch WHERE ip='%s'::inet AND mac='%s'::macaddr",
              IpA.Line(i),Mac.Line(i));
        
        res = PQexec(conn, (const char *)S);
        if ( PQresultStatus(res) != PGRES_TUPLES_OK)
           {fprintf(stderr, "SELECT did not return tuples properly\n");
            PQclear(res);
            exit_nicely(conn,EXIT_FAILURE);
           }

      
       switch (iTup=PQntuples(res))
          {case 1:iCnt=atol(PQgetvalue(res, 0, 4));
                  S.Setf("UPDATE watch SET last_seen=now(), count=%d WHERE \
                          ip='%s'::inet AND mac='%s'::macaddr",
                         iCnt+1,IpA.Line(i),Mac.Line(i));

                  res = PQexec(conn, (const char *)S);
                  if(PQresultStatus(res) != PGRES_COMMAND_OK)
                    {fprintf(stderr, "UPDATE did not return properly\n");
                     PQclear(res);
                     exit_nicely(conn,EXIT_FAILURE);
                    }
                  PQclear(res);
                  break;
           case 0:
                  S.Setf("INSERT INTO watch (ip,mac,last_seen,count) VALUES \
                          ('%s'::inet, '%s'::macaddr, now(), 1)",
                          IpA.Line(i),Mac.Line(i));
                  res = PQexec(conn, (const char *)S);
                  if((j=PQresultStatus(res)) != PGRES_COMMAND_OK)
                    {fprintf(stderr, "INSERT did not return properly\n");
                     PQclear(res);
                     exit_nicely(conn,EXIT_FAILURE);
                    }
                  PQclear(res);
                  break;
           default:fprintf(stderr, "Database integrity corrupt\n");
                   PQclear(res);
                   exit_nicely(conn,EXIT_FAILURE);
          }//switch
     }//for
