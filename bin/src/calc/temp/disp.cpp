...
const char *szRCSID={"$Id: disp.cpp,v 1.1 2001/10/08 15:00:22 xausw Exp xausw $"};
...
{int iO;
 ...
 char szFile[MAXPATH+1];
  ...
  if(!szArgV[optind]) // no file name; is it a pipe ?  
    {CreateTmpFile(szTFile, "Disp", TMPFILE_EXIT);
     if(atexit(RmTmpFile))
  ...
  if( (FileType) iType == FtNONE)
    {fprintf(stderr,"Disp: ERROR-exit\n");
     exit(EXIT_FAILURE);
    }
       ...
       switch ((FileGroup)(int)(iType/10)*10)
        ...
        case NoGROUP:
             ...
             break;
       default:fprintf(stderr,"ERROR Disp: file %s has unsupported type %d\n",
                           szFile,iType);
          exit(EXIT_FAILURE);
      }//switch
  ...
  if(DF->GetFType()==FtERROR || DF->GetFType()==FtILLG)
    {fprintf(stderr,"ERROR Disp: Error reading file %s\n",szFile);
     exit(EXIT_FAILURE);
    }	    

  if( DF->ReadData()>0 || DF->GetCRData()->GetSteps()==0 )
    {fprintf(stderr,"ERROR Disp: Error reading data of %s\n",szFile);
     exit(EXIT_FAILURE);
    }
  if(!DF->GetCRData()->SetColX(ix-1))
    {fprintf(stderr,"ERROR Disp: Illegal x column %d\n",ix);
     exit(EXIT_FAILURE);
    }
   if(!DF->GetCRData()->SetColY(iy-1))
    {fprintf(stderr,"ERROR Disp : Illegal y column %d\n",iy);
     exit(EXIT_FAILURE);
    }
...
{//fprintf(stderr,"temp file:%s\n",szTFile);
 if(szTFile[0])
   {int i=remove(szTFile);
    ...
    if(i)perror("ERROR Disp: Removing tmp-file");
   }
